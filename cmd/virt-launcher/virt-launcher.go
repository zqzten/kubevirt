...
func startDomainEventMonitoring(
	...
	err := notifier.StartDomainNotifier(domainConn, deleteNotificationSent, vmi, domainName, agentStore, qemuAgentSysInterval, qemuAgentFileInterval, qemuAgentUserInterval, qemuAgentVersionInterval, qemuAgentFSFreezeStatusInterval, qemuAgentMemoryInfoInterval, qemuAgentDiskInfoInterval)
	if err != nil {
		panic(err)
	}
}
...
func main() {
	...
	qemuAgentMemoryInfoInterval := pflag.Duration("qemu-agent-memory-info-interval", 30*time.Second, "Interval between consecutive qemu agent calls for memory info  command")
	qemuAgentDiskInfoInterval := pflag.Duration("qemu-agent-disk-info-interval", 30*time.Second, "Interval between consecutive qemu agent calls for disk info  command")
	...
	startDomainEventMonitoring(notifier, *virtShareDir, domainConn, events, vmi, domainName, &agentStore, *qemuAgentSysInterval, *qemuAgentFileInterval, *qemuAgentUserInterval, *qemuAgentVersionInterval, *qemuAgentFSFreezeStatusInterval, *qemuAgentMemoryInfoInterval, *qemuAgentDiskInfoInterval)
