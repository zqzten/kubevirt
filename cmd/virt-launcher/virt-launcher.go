...
func main() {
	...
	qemuAgentMemoryInfoInterval := pflag.Duration("qemu-agent-memory-info-interval", 2*time.Second, "Interval between consecutive qemu agent calls for memory info  command")
	qemuAgentDiskInfoInterval := pflag.Duration("qemu-agent-disk-info-interval", 2*time.Second, "Interval between consecutive qemu agent calls for disk info  command")
